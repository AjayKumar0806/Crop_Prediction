...
def predict_crop(data, data1, data2):
    ...
    # Determine crop types and prepare training data
    data = pd.read_csv("data/file.csv", usecols=range(13))
    Type_new = pd.Series([])

    for i in range(len(data)):
        Type_new[i] = data["Crops"][i]  # Simplified, retain crop names
    ...
    with open('data/test.csv', 'r') as csvfile1:
        ...
        for p in range(len(dataset1)):
            for q in range(4):
                dataset1[p][q] = float(dataset1[p][q])
    ...
    # Get prices of the recommended crops
    data1 = pd.read_csv("data/file.csv", usecols=range(13))
    prices = data1[data1['Crops'] == response]['price'].values

    return response, str(prices[0]), response, str(prices[1]) if len(prices) > 1 else "N/A"
